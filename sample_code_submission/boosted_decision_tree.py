...
classifiers = {
    ...
    "sklearnbdt": ensemble.HistGradientBoostingClassifier(
        ...
        max_iter=256,
        min_samples_leaf=2,
        l2_regularization=1.0392869712218227,
    ),
}
...
if __name__ == "__main__":
    ...
    training_data, train_weights, y_train = (
        train_set["data"],
        train_set["weights"],
        train_set["labels"],
    )
    ...
    train_weights[y_train == 0] *= max(class_weights_train) / class_weights_train[0]
    train_weights[y_train == 1] *= max(class_weights_train) / class_weights_train[1]
    ...
    plt.clf()
    fig, ax = seperation_curve(
        y_test, y_pred_xgb, sample_weight=valid_weights, bins=30, classifier="XGBoost"
    )
    plt.savefig("Histogram of Scores fore XGBoost")
    plt.clf()
    fig, ax = seperation_curve(
        y_test, y_pred_lgb, sample_weight=valid_weights, bins=30, classifier="Lightgbm"
    )
    plt.savefig("Histogram of Scores fore Lightgbm")
    plt.clf()
    fig, ax = seperation_curve(
        y_test, y_pred_skgb, sample_weight=valid_weights, bins=30, classifier="SKlearn GBDT"
    )
